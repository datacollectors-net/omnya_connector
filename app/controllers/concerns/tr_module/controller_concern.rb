module TrModule
  module ControllerConcern
    ...
    included do
      ...
      helper_method :tr_module_module_key,
                    :tr_module_host_app_origins,
                    :tr_module_autonomous_user_guid,
                    :tr_module_autonomous_tenant_id,
    ...
    def assign_current_host_context
      ...
      TrModule::Current.user_guid = current_host_user_guid
      TrModule::Current.tenant_id = current_host_tenant_id
      log_host_context_state(
        event: "host_context.assign.after",
        details: {
          resolved_user_guid: TrModule::Current.user_guid,
          resolved_tenant_id: TrModule::Current.tenant_id
        }
      )
    end
    ...
    def clear_host_context_session!
      ...
      TrModule::Current.user_guid = nil
      TrModule::Current.tenant_id = nil
    ...
    def trusted_embedded_origin_request?
      return false unless Rails.application.config.x.tr_module.allow_trusted_origin_csrf_bypass

      origin = request.headers["Origin"].to_s
      return false if origin.blank?
      return false if origin == request.base_url

      tr_module_host_app_origins.include?(origin)
    end

    def tr_module_module_key
      Rails.application.config.x.tr_module.module_key
    end

    def tr_module_host_app_origins
      Rails.application.config.x.tr_module.host_app_origins
    end

    def tr_module_autonomous_user_guid
      Rails.application.config.x.tr_module.autonomous_user_guid
    end

    def tr_module_autonomous_tenant_id
      Rails.application.config.x.tr_module.autonomous_tenant_id
    end
      ...
      Rails.logger.info(
        {
          ...
          autonomous_user_guid: tr_module_autonomous_user_guid,
          autonomous_tenant_id: tr_module_autonomous_tenant_id
        }.merge(details)
      )
    end
  end
