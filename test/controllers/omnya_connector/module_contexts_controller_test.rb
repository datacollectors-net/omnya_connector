...
class OmnyaConnector::ModuleContextsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "re-sync without session continuity uses fallback fingerprint and reports unchanged" do
    first_session = open_session
    second_session = open_session

    with_stubbed_host_context({ "user" => { "guid" => "same-user" }, "tenant" => { "id" => 321 } }) do
      first_session.post omnya_connector.module_context_url, params: {
        module_context: {
          token: "token-same-a",
          context_endpoint: "https://host.example.test/external_modules/context",
          module_key: Rails.application.config.x.omnya_connector.module_key
    ...
    assert_equal 204, first_session.response.status
    assert_equal "1", first_session.response.headers["X-Omnya-Context-Changed"]

    previous_fingerprint = first_session.response.headers["X-Omnya-Context-Fingerprint"]

    with_stubbed_host_context({ "user" => { "guid" => "same-user" }, "tenant" => { "id" => 321 } }) do
      second_session.post omnya_connector.module_context_url,
        params: {
          module_context: {
            token: "token-same-b",
            context_endpoint: "https://host.example.test/external_modules/context",
        ...
        headers: {
          "X-Omnya-Context-Previous-Fingerprint" => previous_fingerprint
        }
    end

    assert_equal 204, second_session.response.status
    assert_equal "0", second_session.response.headers["X-Omnya-Context-Changed"]
  end

  test "re-sync without session continuity keeps changed when fallback fingerprint differs" do
    isolated_session = open_session

    with_stubbed_host_context({ "user" => { "guid" => "same-user" }, "tenant" => { "id" => 321 } }) do
      isolated_session.post omnya_connector.module_context_url,
        params: {
          module_context: {
            token: "token-same-c",
            context_endpoint: "https://host.example.test/external_modules/context",
        ...
        headers: {
          "X-Omnya-Context-Previous-Fingerprint" => "same-user:999"
        }
    end

    assert_equal 204, isolated_session.response.status
    assert_equal "1", isolated_session.response.headers["X-Omnya-Context-Changed"]
  end

  test "re-sync without session continuity ignores malformed fallback fingerprint" do
    isolated_session = open_session
    ...
    with_stubbed_host_context({ "user" => { "guid" => "same-user" }, "tenant" => { "id" => 321 } }) do
      isolated_session.post omnya_connector.module_context_url,
        ...
        headers: {
          "X-Omnya-Context-Previous-Fingerprint" => "malformed"
        }
    end
