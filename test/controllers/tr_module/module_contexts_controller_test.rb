...
class TrModule::ModuleContextsControllerTest < ActionDispatch::IntegrationTest
  test "creates context from valid payload" do
    ...
    with_stubbed_host_context(context_payload) do
      post tr_module.module_context_url, params: {
        module_context: {
          token: "token-1",
          context_endpoint: "https://host.example.test/external_modules/context",
          module_key: Rails.application.config.x.tr_module.module_key
        }
      }
    end
  ...
  test "destroy clears host context" do
    ...
    delete tr_module.module_context_url
    assert_response :no_content
  end
  ...
  def with_stubbed_host_context(payload)
    original_call = TrModule::HostContextFetcher.method(:call)
    TrModule::HostContextFetcher.define_singleton_method(:call) { |**_args| payload }
    yield
  ensure
    TrModule::HostContextFetcher.define_singleton_method(:call, original_call)
  end

  def with_stubbed_host_context_error(message)
    original_call = TrModule::HostContextFetcher.method(:call)
    TrModule::HostContextFetcher.define_singleton_method(:call) do |**_args|
      raise TrModule::HostContextFetcher::Error, message
    end
    yield
  ensure
    TrModule::HostContextFetcher.define_singleton_method(:call, original_call)
